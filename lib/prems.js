...
const sock = makeWASocket({
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'open') {
      ...
      if (!globalThis.db.data.settings[botDir].botprem) {
...
/* client.sendMessage(m.chat, { text: `✎ Has conectado un nuevo Socket de tipo *Premium*.` }, { quoted: m })
delete commandFlags[senderId]
      ...
      console.log(chalk.gray(`[ ✿  ]  PREM-BOT conectado: ${sock.userId}`))
    }
    ...
    if (connection === 'close') {
      ...
      if ([401, 403].includes(reason)) {
        if (intentos < 5) {
          console.log(
            chalk.gray(
              `[ ✿  ]  PREM-BOT ${botId} Conexión cerrada (código ${reason}) intento ${intentos}/5 → Reintentando...`,
            ),
          )
          setTimeout(() => {
          ...
          console.log(
            chalk.gray(`[ ✿  ]  PREM-BOT ${botId} Falló tras 5 intentos. Eliminando sesión.`),
          )
          try {
      ...
      if (
        ...
        // console.log(chalk.gray(`[ ✿  ]  PREM-BOT ${botId} desconectado → Reconectando...`))
        setTimeout(() => {
          startPremBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
        }, 3000)
        return
