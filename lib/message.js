...
export async function smsg(client, m, store) {
  ...
  if (m.key) {
    ...
    m.isBot =
      ...
      ) || /(.)\1{5,}|[^a-zA-Z0-9]|[^0-9A-F]/.test(m.id) || false
  ...
  if (m.message) {
    ...
    m.body =
      m.message?.conversation || m.msg?.text || m.msg?.conversation ||
      m.msg?.caption || m.msg?.selectedButtonId ||
    ...
    // ── AGREGADO DE ALYA #2: m.usedPrefix con GraphemeSplitter ──
    const idBot = client.user.id.split(':')[0] + '@s.whatsapp.net'
    const config = global.db?.data?.settings?.[idBot] || {}
    const splitter = new GraphemeSplitter()
    let activePrefixes = []
    if (config.prefijo === true) { activePrefixes = [] }
    else if (Array.isArray(config.prefijo)) { activePrefixes = config.prefijo }
    else if (typeof config.prefijo === 'string') { activePrefixes = splitter.splitGraphemes(config.prefijo) }
    else { activePrefixes = ['#', '/'] }
    m.usedPrefix = ''
    ...
    m.expiration =
      m.msg?.contextInfo?.expiration || m?.metadata?.ephemeralDuration ||
      client?.messages?.[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0
    m.timestamp =
      (typeof m.messageTimestamp === 'number' ? m.messageTimestamp
        : m.messageTimestamp.low ? m.messageTimestamp.low
    ...
    if (m.quoted) {
      ...
      m.quoted.text =
        m.quoted.caption || m.quoted.conversation || m.quoted.contentText ||
      ...
      m.quoted.body =
        m.quoted.msg?.text || m.quoted.msg?.caption || m.quoted?.message?.conversation ||
        m.quoted.msg?.selectedButtonId || m.quoted.msg?.singleSelectReply?.selectedRowId ||
        m.quoted.msg?.selectedId || m.quoted.msg?.contentText ||
      ...
      // ── AGREGADO DE ALYA #3: m.quoted.usedPrefix con activePrefixes ──
      let quotedPrefix = ''
      if (activePrefixes.length > 0) {
        for (const p of activePrefixes) {
          if (m.quoted.body?.startsWith(p)) { quotedPrefix = p; break }
        }
      }
  ...
  client.getName = (jid, withoutContact = false) => {
    ...
    if (id.endsWith('@g.us'))
      ...
      v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' }
        : id === client.decodeJid(client.user.jid) ? client.user
        : store.contacts[id] || {}
  ...
  client.appenTextMessage = async (text, chatUpdate) => {
    let messages = await generateWAMessage(
      m.chat, { text: text, mentions: m.mentionedJid },
      { userJid: client.user.id, quoted: m.quoted && m.quoted.fakeObj },
    )
  ...
  client.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
    ...
    else { buffer = await imageToWebp(buffer) }